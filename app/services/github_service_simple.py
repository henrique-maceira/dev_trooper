...
class SimpleGitHubService:
    """Serviço Git simplificado sem dependência do PyGithub"""
    ...
    def open_pr(self, full_repo_name: str, title: str, head_branch: str, base: str, body: str) -> Optional[str]:
        """Simula criação de PR (retorna URL simulada)"""
        try:
            # Como não temos PyGithub, retornamos uma URL simulada
            pr_url = f"https://github.com/{full_repo_name}/pull/123"
            logger.info(f"PR simulado criado: {pr_url}")
            return pr_url
...
# Instância global
github_service = SimpleGitHubService()
